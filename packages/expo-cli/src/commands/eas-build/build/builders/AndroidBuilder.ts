...
const hasApplyLine = (content: string, applyLine: string) =>
  content
    .split('\n')
...
class AndroidBuilder implements Builder<Platform.Android> {
  ...
  private async isProjectConfiguredAsync(): Promise<boolean> {
    ...
    const isUpdatesConfigured = await isUpdatesConfiguredAndroidAsync(
      this.ctx.commandCtx.projectDir
    );

    return hasEasGradleApply && hasEasGradleFile && isUpdatesConfigured;
  }
